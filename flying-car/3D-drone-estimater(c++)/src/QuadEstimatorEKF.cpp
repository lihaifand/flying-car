...
void QuadEstimatorEKF::UpdateFromIMU(V3F accel, V3F gyro)
{
  ...
  Quaternion<float> attitudeUpdated = attitude.IntegrateBodyRate(gyro, dtIMU);
  
  float predictedPitch = attitudeUpdated.Pitch();
  float predictedRoll = attitudeUpdated.Roll();
  ekfState(6) = attitudeUpdated.Yaw();  // yaw
...
VectorXf QuadEstimatorEKF::PredictState(VectorXf curState, float dt, V3F accel, V3F gyro)
{
  ...
  predictedState(3) += globalAccel.x*dt;
  predictedState(4) += globalAccel.y*dt;
  predictedState(5) += (globalAccel.z-CONST_GRAVITY)*dt;
  /////////////////////////////// END STUDENT CODE ////////////////////////////
...
void QuadEstimatorEKF::Update(VectorXf& z, MatrixXf& H, MatrixXf& R, VectorXf& zFromX)
{
  ...
  toInvert = H*ekfCov*Ht + R;
  MatrixXf K = ekfCov * Ht * toInvert.inverse();
